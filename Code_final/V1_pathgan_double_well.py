...
mb_size = 10000
D_h1_dim=10
D_h2_dim=10 #layers and nodes of discriminator
Z_dim = 2
G_h1_dim=4
G_h2_dim=4 #layers and nodes of generator
#G_h3_dim=4
...
toy_data = genfromtxt('V1_data/double_well/double_well.csv', delimiter=',', dtype='float32')
...
if not os.path.exists('V1_results/double_well/csv/'):
    os.makedirs('V1_results/double_well/csv/')
...
with open('V1_results/double_well/csv/pathgan_beta_'+str(beta)+'_'+str(iteration)+'_nonpar.csv', "w") as output: # _nonpar
...
NoPoints = 1000
V = np.zeros(shape=(NoPoints, X_dim))
...
with open('V1_results/double_well/csv/pathgan_beta_'+str(beta)+'_'+str(iteration)+'_drift_nonpar.csv', "w") as output:
    writer = csv.writer(output, lineterminator='\n')
    for val in V:
        writer.writerow(val)
